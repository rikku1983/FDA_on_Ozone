...
warp=function(fd, rfd){
  ...
  basisf=create.fourier.basis(c(1,53), 35)
  frange=fd$basis$rangeval
  xf=seq(frange[1],frange[2],len=n)
  xfs=(xf-frange[1])/(frange[2]-frange[1])
  newx=eval.fd(xfs,rfd)*(frange[2]-frange[1])+frange[1]
  newx[1]=frange[1]
  newx[n]=frange[2]
  y=eval.fd(newx, fd)
...
energySRSF2=function(f,g,k,l,i,j){
  ...
  fidx=round(seq(k*m/101, i*m/101, len=neval))
  #gidx=round(l*m/101):round(j*m/101)
  gidx=round(seq(l*m/101, j*m/101, len=neval))
  cost=sum((f[fidx]-g[gidx]*sqrt(slope))^2)/neval
  return(cost)
}
